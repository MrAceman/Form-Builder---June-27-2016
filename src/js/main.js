...
function textareaTemplate(field) {
    return `<div class="form_textarea_field"><form>
  <label for="${field.label}" class = "form_label">${field.label}</label>
...
function selectTemplate(field) {
    var selectString = ""
    var optionsString = ""
    selectString = `<div class="form_select_field"><form><select name ="language_select">`
    field.options.forEach(function(option) {
        optionsString += `<option value = "${option.value}">${option.label}</option>`
    })
    return selectString += optionsString + `</select></form></div>`;
};
